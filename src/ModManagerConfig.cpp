...
{
    JsonUtils::create_or_update_json(path + PROFILE_FILE_NAME, {"LastSelectedGame"}, selection, true);
}

int ModManagerConfig::get_last_selected_game(const std::string &path)
{
    int lastSelectedGameIndex = JsonUtils::get_integer_value(path + PROFILE_FILE_NAME, {"LastSelectedGame"});
...
{
    JsonUtils::create_or_update_json(path + PROFILE_FILE_NAME, {"Games", gameSelection + "GamePath"}, pathSelection, true);
}

std::string ModManagerConfig::get_game_path(const std::string &path, const std::string &selection)
{
    std::string gamePath = JsonUtils::get_string_value(path + PROFILE_FILE_NAME, {"Games", selection + "GamePath"});
