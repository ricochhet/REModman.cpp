...
void ModManagerConfig::set_selected_profile(const std::string& path) {
    JsonUtils::create_or_update_json(path + REMM_PROFILE_FILE_NAME, {REMM_CONF_SELECTED_PROFILE}, path, true);
}
...
void ModManagerConfig::set_game_path(
    ...
    JsonUtils::create_or_update_json(
        path + REMM_PROFILE_FILE_NAME, {REMM_CONF_GAMES, gameSelection + REMM_CONF_GAME_PATH}, pathSelection, true
    );
}

std::string ModManagerConfig::get_game_path(const std::string& path, const std::string& selection) {
    std::string gamePath =
        JsonUtils::get_string_value(path + REMM_PROFILE_FILE_NAME, {REMM_CONF_GAMES, selection + REMM_CONF_GAME_PATH});
